...
(function () {
  ...
  function zeigeAkte() {
    ...
    const saisons = {
      1: "SAISON 1 – ANKUNFT", 2: "SAISON 2 – DER PLAN", 3: "SAISON 3 – RÜCKBLENDE",
      4: "SAISON 4 – ZUGRIFF", 5: "SAISON 5 – LE RETOUR &middot; 3. LERNJAHR"
    };
    let inhalt = "";
    let letzteSaison = 0;
    ...
    zeichnen(`${kopf(`<span class="chip">${Math.round(DP.gesamtFortschritt() * 100)}% gesamt</span>`)}
      <h1>Die Akte</h1>
      <p class="klein grau">32 Module – zwei Jahre Rückstand plus der Stoff, der dieses Jahr dran ist. Jeder Balken zeigt, wie sicher das Thema sitzt. Tipp ein Modul an, um Regeln und Wörter nachzuschlagen.</p>
      ${inhalt}
  ...
  function zeigeBericht() {
    ...
    const saisonStaerke = [1, 2, 3, 4, 5].map(n => {
  ...
  function zeigeEinstellungen() {
    ...
    auf("#sicherungRaus", "click", () => {
      const text = DP.exportieren();
      q("#sicherungsfeld").innerHTML = `
        <div class="klein grau mt">Dieser Text ist dein kompletter Spielstand.
        Kopier ihn und schick ihn dir selbst – per Mail, Notiz, egal.</div>
        <textarea class="eingabe mt" id="sicherungText" rows="4" readonly
          style="font-size:11px;resize:vertical">${esc(text)}</textarea>
        <button class="btn btn-geist mt" id="kopierSicherung">In die Zwischenablage kopieren</button>`;
      const feld = q("#sicherungText");
      auf("#kopierSicherung", "click", () => {
        feld.removeAttribute("readonly");
        feld.select();
        try {
          navigator.clipboard.writeText(text);
          q("#kopierSicherung").textContent = "Kopiert.";
        } catch (e) {
          q("#kopierSicherung").textContent = "Markiert – jetzt selbst kopieren.";
        }
    ...
    auf("#sicherungRein", "click", () => {
      q("#sicherungsfeld").innerHTML = `
        <div class="klein grau mt">Sicherungstext hier einfügen. Achtung: der aktuelle
        Stand auf diesem Gerät wird dabei überschrieben.</div>
        <textarea class="eingabe mt" id="einspielText" rows="4"
          style="font-size:11px;resize:vertical" placeholder="Sicherungstext einfügen"></textarea>
        <button class="btn btn-geist mt" id="einspielen">Diesen Stand übernehmen</button>`;
      auf("#einspielen", "click", () => {
        const text = q("#einspielText").value;
        if (!text.trim()) return;
        if (!confirm("Der Fortschritt auf diesem Gerät wird ersetzt. Weitermachen?")) return;
        const ergebnis = DP.importieren(text);
        if (!ergebnis.ok) {
          q("#sicherungsfeld").innerHTML = `<div class="rueckmeldung schlecht mt">
            <div class="kopfzeile">Ging nicht</div><div>${esc(ergebnis.grund)}</div></div>`;
          return;
        }
        alert("Übernommen: " + ergebnis.codename + ", " + ergebnis.module +
              " abgeschlossene Module, " + ergebnis.karten + " Karten.");
        zeigeBasis();
      });
    });
